...
<style>
    ...
    .fc-bg-event .fc-event-title{
        ...
        box-shadow: 0px 0px 7px rgb(0 0 0 / 99%) !important;
    }
    ...
    .legend span { border: none; float: left; width: 12px; height: 12px; margin: 6px; }
</style>
                ...
                <div class="col-xl-12 col-md-12 col-lg-12 col-sm-12 col-12">
                    <div class="view_profile_wrapper float_left">
                        ...
                        <div class="row mt-4">
                            <ul class="legend">
                                <?php 
                                    for($i=0; $i<sizeof($rm_color); $i++){
                                        echo '<li><span style="background-color: '.$rm_color[$i].'"></span> ('.$rm_number[$i].') '.$rm_name[$i].'</li>';
                                    }
                                ?>
                            </ul>
                        </div>
    ...
    $(document).ready(function(){
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            ?> 
                {
                    ...
                    <?php 
                        if($full_booked_date[$i] == 1) echo "color: '#132a13',";
                        else echo "color: '#E8C4C4',";
                    ?>
                },
            <?php }} ?>
                <?php for ($i=0; $i <count($booking_id); $i++) { ?> 
                    {
                        id : '<?php echo $booking_id[$i]; ?>',
                        title: '#<?php echo $booking_code[$i] .' : ('.$room_number[$i].') '. $room_name[$i]; ?>',
                        start: '<?php echo $arrival_date[$i]; ?>',
                        end: '<?php echo $departure_date[$i]; ?>',
                        color: '<?php echo $room_color[$i]; ?>',
                    },
                <?php } ?>
            ],
            eventDidMount: function(info) {
                ...
                // info.el.append("<br/>" + info.event.extendedProps.description);
                // $(info.el).tooltip({
            ...
            eventClick: function (info) {
                $.ajax({
                    url: 'index.php?components=admin&action=ajax_get_booking_data_calendar_even_select',
                    type: 'POST',
                    async: false,
        ...
        function load_data(is_date_search='', date_type='', start_date='', end_date='',start, length){
            var dataTable = $('#upcoming_booking_data_table').DataTable({
                language: {
                        searchPlaceholder: "Search Booking Code"
                },
                columnDefs: [
                    {targets: [1,2,4,9],className: 'text-left'},
                    {targets: [3,7,8],className: 'text-right'},
                    {targets: [6,5],className: 'text-center'},
