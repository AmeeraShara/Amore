<?php
    function getUpcomingBookingDataAjax(){
        $data = array();
        $today = dateNow();
        $query = $from = $to = $query_total_records = $number_filter_row = $records_total = $search_qry = '';
        include('config.php');

        $sort_column = array("rb.`id`","rb.`booking_code`", "rb.`first_name`", "c.`country`","rb.`arrival_date`","rb.`departure_date`","rb.`adults`","rb.`children`","rbmp.`booking_meal_plan_id`","rb.`email`","rb.`mobile`");
        $search_columns = "rb.`id`,rb.`booking_code`,rb.`first_name`,rb.`last_name`,c.`country_name`,rb.`arrival_date`,rb.`departure_date`,rb.`adults`,rb.`children`,rb.`email`,rb.`mobile`, GROUP_CONCAT(mp.`name`) AS `meals`";
        $joins_qry = "LEFT JOIN `rooms_booking` rb ON r.`id` = rb.`room_id`
        ...
        $where_qry = "rb.`arrival_date` >= '$today' AND rb.`status`='1'";
        $date_qry = "";
        $search_qry = "";
        $group_qry = "GROUP BY rb.`id`";
        $order_qry = "";
        $limit_qry = "";

        $query_total_records = "SELECT `id` FROM `rooms_booking` WHERE  `status` = '1'";
        $records_total = mysqli_num_rows(mysqli_query($conn,$query_total_records));  // total number of records count

        if(isset($_POST["search"]["value"]) && ($_POST["search"]["value"] != '')){ // search by code or name
            $value = $_POST["search"]["value"];
            $search_qry = " AND (rb.`booking_code` LIKE '%".$value."%' OR CONCAT(rb.`first_name`, '', rb.`last_name`)  LIKE '%".$value."%')";
        }
        ...
        if((isset($_POST["is_date_search"])) &&  ($_POST["is_date_search"] == "yes") && (isset($_POST["date_type"])) && ($_POST["date_type"] == 'arrival') && ($_POST["start_date"] != '') && ($_POST["end_date"] != '')){ // date filter
            ...
            $to = $_POST["end_date"];
            $date_qry = "AND DATE(rb.`arrival_date`) BETWEEN DATE('$from') AND DATE('$to')";
        }
        ...
        if((isset($_POST["is_date_search"])) &&  ($_POST["is_date_search"] == "yes") && (isset($_POST["date_type"])) && ($_POST["date_type"] == 'departure') && ($_POST["start_date"] != '') && ($_POST["end_date"] != '')){ // date filter
            $from = $_POST["start_date"];
            $to = $_POST["end_date"];
            $date_qry = "AND DATE(rb.`departure_date`) BETWEEN DATE('$from') AND DATE('$to')";
        }

        if((isset($_POST['order'])) && ($_POST["order"] != '')){ // sorting
            $order_qry = ' ORDER BY '.$sort_column[$_POST['order']['0']['column']].' '.$_POST['order']['0']['dir'].' ';
        }else{
            $order_qry = ' ORDER BY rb.`arrival_date` ASC ';
        }

        if($_POST['length'] != -1){ // page limit
            $limit_qry = ' LIMIT ' . $_POST['start'] . ', ' . $_POST['length'];
        }
        
        $query = "SELECT $search_columns FROM `rooms` r $joins_qry WHERE $where_qry $search_qry $date_qry $group_qry $order_qry $limit_qry";
    ...
    function getRoomDataAjax(){
        ...
        $query = "SELECT
        rb.`booking_code`, rb.`arrival_date`, rb.`departure_date`, rb.`adults`, rb.`children`, rb.`first_name`,
        rb.`last_name`, rb.`total`, rb.`email`, rb.`mobile`, rb.`city`, rb.`address`, rb.`extra_note`, rb.`booked_at`,
        rb.`single_night_price`, r.`name`, c.`country_name`, r.`room_number`, GROUP_CONCAT(mp.`name`)
        FROM `rooms` r
        LEFT JOIN `rooms_booking` rb ON r.`id` = rb.`room_id`
        LEFT JOIN `countries` c ON c.`id` = rb.`country`
        LEFT JOIN `room_booked_meal_plans` rbmp ON rbmp.`guest_id` = rb.`id`
        LEFT JOIN `room_booking_meal_plans` mp ON mp.`id` = rbmp.`booking_meal_plan_id`
        WHERE rb.`status`='1' AND rb.`id`='$id'";
        $result = mysqli_query($conn,$query);
        $row=mysqli_fetch_row($result);
